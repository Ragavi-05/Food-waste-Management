...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (wasteTableBody) {
        // helper to generate random entries for demonstration
        function generateDummyData(count) {
            const meals = ['Breakfast', 'Lunch', 'Dinner'];
            const items = ['Rice', 'Curry', 'Bread', 'Vegetables', 'Soup', 'Salad'];
            const locations = ['Hotel', 'Restaurant', 'Canteen', 'Hostel'];
            const reasons = ['Over Preparation', 'Leftovers', 'Poor Quality', 'Spillage', 'Expired'];
            const data = [];
            ...
            for (let i = 0; i < count; i++) {
                ...
                const prepared = Math.floor(Math.random() * 50) + 1;
                const consumed = Math.floor(Math.random() * prepared);
                const waste = prepared - consumed;
                const location = locations[Math.floor(Math.random() * locations.length)];
                const reason = reasons[Math.floor(Math.random() * reasons.length)];
                data.push({ date: dateStr, meal, item, prepared, consumed, waste, location, reason });
            }
            return data;
        }

        // generate 500 records (plus a few constant ones if desired)
